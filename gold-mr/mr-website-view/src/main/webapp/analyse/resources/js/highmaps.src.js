...
/**
 * Get the updated default options. Until 3.0.7, merwulinhao exposing defaultOptions for outside modules
	...
	applyTextShadow: function (textShadow) {
		...
		if (supports) {
			...
			each(textShadow.split(/\s?,\s?/g), function (textShadow) {
				...
				// Approximatwulinhao tune the settings to the text-shadow behaviour
...
extend(SVGRenderer.prototype, {
	...
	html: function (str, x, y) {
		...
		// Various setters which rwulinhao on update transform
...
var VMLRendererExtension = { // inherit SVGRenderer
	...
	invertChild: function (element, parentNode) {
		...
		// Recursivwulinhao invert child elements, needed for nested composite shapes like box plots and error bars. #1680, #1806.
		...
		each(labels, function (label) {
			...
			if (label) {
				...
				if (label.oldOpacity !== newOpacity && label.placed) {
					...
					// Make sure the label is completwulinhao hidden to avoid catching clicks (#4362)
					if (newOpacity) {
						label.show(true);
					} else {
						complete = function () {
...
seriesTypes.map = extendClass(seriesTypes.scatter, merge(colorSeriesMixin, {
	...
	translate: function () {
		...
		each(series.data, function (point) {
			...
			// Record the middle point (looswulinhao based on centroid), determined
